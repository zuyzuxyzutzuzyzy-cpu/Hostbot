...
OWNER_ID = 5319770650 # Replace with your Owner ID
ADMIN_ID = 5319770650 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@MahiByLike' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/+Sv0ZgA47FfwzZDBl' # Replace with your update channel link
...
def attempt_install_pip(module_name, message):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Admin-only Force Install button
            cb_pkg = package_name[:40]  # keep callback_data within 64 bytes
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(
                f"📦 Force Install `{cb_pkg}`",
                callback_data=f"finstall_{cb_pkg}"
            ))
            bot.reply_to(message, error_msg + "\n\n👑 *Admins can force-install using the button below:*",
...
def force_install_pip_callback(call):
    """Admin-only: force install a Python package using --break-system-packages."""
    ...
    logger.info(f"Force install by admin {user_id}: {' '.join(command)}")
    try:
        result = subprocess.run(command, capture_output=True, text=True, check=False,
                                encoding='utf-8', errors='ignore')
        if result.returncode == 0:
            logger.info(f"Force installed {package_name}. Output:\n{result.stdout}")
            bot.send_message(call.message.chat.id,
                             f"✅ Package `{package_name}` force-installed successfully!\n"
                             f"Ab script dobara start karein.",
                             parse_mode='Markdown')
        else:
            err = (result.stderr or result.stdout)[:3000]
            bot.send_message(call.message.chat.id,
                             f"❌ Force install bhi fail hua `{package_name}`.\nLog:\n```\n{err}\n```",
                             parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Force install error for {package_name}: {e}", exc_info=True)
...
def _logic_manual_install(message):
    """Reply-keyboard handler for Manual Install — admin only."""
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Sirf admins ye kar sakte hain!")
        return
...
def manual_install_init_callback(call):
    """Inline-button handler for Manual Install — admin only."""
...
def process_manual_install(message):
    """Process the package name typed by admin and install it."""
    user_id = message.from_user.id
    if user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Sirf admins ye kar sakte hain!")
        return
    ...
    if text.lower() == '/cancel':
        ...
        return
    if not text:
        bot.reply_to(message, "⚠️ Koi package naam nahi diya. Dobara try karo.")
        msg = bot.send_message(message.chat.id, "📦 Package naam bhejo ya /cancel likho:")
        bot.register_next_step_handler(msg, process_manual_install)
        return
    # Allow multiple packages separated by space (e.g. "aiohttp requests")
    packages = text.split()
    ...
    logger.info(f"Manual install by admin {user_id}: {' '.join(command)}")
    try:
        result = subprocess.run(command, capture_output=True, text=True, check=False,
                                encoding='utf-8', errors='ignore')
        if result.returncode == 0:
            logger.info(f"Manual install OK: {packages}. Output:\n{result.stdout}")
            bot.edit_message_text(
                f"✅ *Package(s) install ho gaye!*\n`{' '.join(packages)}`\n\nAb script dobara start karo.",
        ...
        logger.error(f"Manual install error for {packages}: {e}", exc_info=True)
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'manual_install': admin_required_callback(call, manual_install_init_callback)
        elif data.startswith('finstall_'): force_install_pip_callback(call)
